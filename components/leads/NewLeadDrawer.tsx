...
const control =
  "w-full rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3 text-white outline-none focus:border-orange-500";
...
function DynamicField({
  ...
  if (field.type === "textarea") {
    return (
      <label className="space-y-2 text-sm text-zinc-400 sm:col-span-2">
        <span>{field.label}</span>
...
export function NewLeadDrawer({
  ...
  const [serviceType, setServiceType] =
    useState<OpportunityServiceType>(
      "USINA_SOLAR"
    );
  ...
  async function submit(
    ...
    if (!ownerId) {
      setError(
        "Selecione o responsável pela oportunidade."
      );
      return;
    }
    ...
    const payload = {
      ...
      distributor:
        serviceTypeConfig[serviceType]
          .solar
          ? serviceDetails.distributor
          : null,
      consumerUnit:
        serviceTypeConfig[serviceType]
          .solar
          ? serviceDetails.consumerUnit
          : null,
      consumptionKwh:
        serviceTypeConfig[serviceType]
          .solar
          ? Number(
              serviceDetails.monthlyConsumption ||
                0
            )
          : null,
      demandKw:
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60">
      <div className="h-full w-full max-w-3xl overflow-y-auto border-l border-zinc-800 bg-zinc-950 p-6 shadow-2xl">
        <header className="mb-8 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-400">
              Dados básicos e especificações compatíveis com o serviço.
            </p>
          </div>
        ...
        >
          <section>
            <h3 className="mb-4 text-xs font-bold uppercase tracking-[.18em] text-orange-400">
              Dados básicos
            </h3>
              ...
              <label className="space-y-2 text-sm text-zinc-400 sm:col-span-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setServiceType(
                      event.target
                        .value as OpportunityServiceType
                    )
                  }
                  className={control}
                >
                  {OPPORTUNITY_SERVICE_TYPES.map(
                    (type) => (
                      <option
                        key={type}
                        value={type}
          ...
          <section>
            <h3 className="mb-1 text-xs font-bold uppercase tracking-[.18em] text-orange-400">
          ...
          <footer className="flex justify-end gap-3 border-t border-zinc-800 pt-5">
            <button
              type="button"
              onClick={onClose}
