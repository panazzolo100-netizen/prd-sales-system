...
export function UsersManagement() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(body.error ?? "Erro ao listar usuários.");
      }
  ...
  async function emailAction(user: ManagedUser) {
    try {
      await action(
        "/api/settings/users",
        {
      ...
      setMessage(
        error instanceof Error ? error.message : "E-mail não enviado."
      );
    }
  }
  ...
  async function remove(user: ManagedUser) {
    if (
      !window.confirm(
        `Excluir ${user.name}? Esta ação não pode ser desfeita.`
      )
    ) {
      return;
    }
    ...
    try {
      await action(
        `/api/settings/users?id=${encodeURIComponent(user.id)}`,
        {
          method: "DELETE",
        },
        "Usuário excluído."
      ...
      setMessage(
        error instanceof Error ? error.message : "Usuário não excluído."
      );
    }
  }
      ...
      <section className="flex flex-col gap-5 rounded-[28px] border border-white/[0.07] bg-gradient-to-br from-zinc-900 to-violet-950/20 p-7 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-zinc-400">
            Acessos, perfis e clientes vinculados da empresa.
          </p>
        </div>

        <button
          onClick={() => setEditing("new")}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 px-5 py-3 font-bold hover:bg-orange-600"
        >
          <Plus size={18} />
          Novo usuário
        </button>
        ...
        >
          <div className="relative flex-1">
            ...
            <input
              ...
              className="h-12 w-full rounded-xl border border-zinc-800 bg-zinc-950 pl-11 pr-4 outline-none focus:border-orange-500"
            />
          </div>

          <button className="rounded-xl border border-zinc-700 px-5 py-3 font-bold hover:bg-zinc-800">
            Pesquisar
          </button>
          ...
          <button
            type="button"
            onClick={() => void load()}
            className="inline-flex items-center justify-center rounded-xl border border-zinc-700 px-4 hover:bg-zinc-800"
...
function UserDialog({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error ?? "Usuário não salvo.");
      }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
      <form
        onSubmit={submit}
        className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-white/10 bg-zinc-900 p-6 shadow-2xl"
      >
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          {!user && (
            <>
              ...
              <label className="flex items-start gap-3 rounded-xl border border-zinc-800 bg-zinc-950/70 p-4 sm:col-span-2">
                <input
                  type="checkbox"
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-zinc-500">
                    O usuário entrará com a senha temporária e deverá
                    cadastrar uma nova senha antes de acessar o sistema.
                  </span>
                </span>
              </label>
            </>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            disabled={saving}
            className="rounded-xl bg-orange-500 px-5 py-3 font-bold hover:bg-orange-600 disabled:opacity-50"
          >
...
const inputClass =
  "h-12 w-full rounded-xl border border-zinc-800 bg-zinc-950 px-4 text-white outline-none focus:border-orange-500 disabled:opacity-60";