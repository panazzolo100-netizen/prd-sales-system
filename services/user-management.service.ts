...
async function validateClient(
  ...
  if (!(await findCompanyClient(clientId, companyId))) {
    throw new AccessDeniedError(
      "O cliente não pertence à empresa autenticada."
    );
  }
...
async function assertCanRemoveExecutive(input: {
  ...
  if (input.targetRole !== "EXECUTIVO") {
    return;
  }

  const executives = await countCompanyExecutives(
    input.companyId
  );
...
export async function getUserManagementData(input: {
  ...
  const pageSize = Math.min(
    ...
    Math.max(
      1,
      Math.trunc(input.pageSize ?? 10)
    )
  );

  const [{ users, total }, clients, authUsers] =
    await Promise.all([
      listManagedUsers({
  ...
  return {
    users: users.map((user) => {
      const authUser = authByEmail.get(
        user.email.toLowerCase()
      );
...
export async function createCompanyUser(input: {
  ...
  if (password.length < MINIMUM_PASSWORD_LENGTH) {
    throw new Error(
      `A senha temporária deve ter pelo menos ${MINIMUM_PASSWORD_LENGTH} caracteres.`
    );
  }
  ...
  if (await findAuthUserByEmail(email)) {
    throw new Error(
      "Já existe um usuário no Supabase Auth com este e-mail."
    );
  }
...
export async function updateCompanyUser(input: {
  ...
  const target = await findManagedUser(
    input.id,
    current.companyId
  );
...
export async function setCompanyUserActive(
  ...
  const { error } =
    await createSupabaseAdmin().auth.admin.updateUserById(
      authUser.id,
      {
        ban_duration: active
          ? "none"
...
export async function sendCompanyUserPasswordEmail(
  id: string
) {
  const current = await requirePermission(
    PERMISSIONS.ADMINISTRATION
  ...
  const { error } =
    await createSupabaseAdmin().auth.resetPasswordForEmail(
      target.email,
      {
...
export async function removeCompanyUser(id: string) {
  ...
  if (authUser) {
    const { error } =
      await createSupabaseAdmin().auth.admin.deleteUser(
        authUser.id
      );
  ...
  const deleted = await deleteManagedUser(
    target.id,
    current.companyId
  );
  ...
  if (!deleted) {
    throw new Error(
      "O usuário recebeu novos vínculos e não pode ser excluído."
    );
  }
