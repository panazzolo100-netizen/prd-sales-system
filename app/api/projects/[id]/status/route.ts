...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const project = await changeProjectStatus(id, body.status, body.expectedUpdatedAt ? new Date(body.expectedUpdatedAt) : undefined);
    ...
    return NextResponse.json({ error: message === "CONFLICT" ? "O projeto foi alterado em outra sessão. Atualize a página." : message }, { status: message === "CONFLICT" ? 409 : message.includes("não encontrado") ? 404 : 400 });
  }
}
